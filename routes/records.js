...
router.get('/searchrecords/', function(req, res) {
    ...
    if (req.query.Limit != null || req.query.Skip != null) {
        ...
        collection.count(searchRequest, function (error, count) {
            console.log(error, count);
            collectionCount = count;
            collection.find(searchRequest, searchOptions,function(e,docs){
                console.log("Search request results length = " + docs.length);
                var searchResult = {};
                searchResult.totalCount = collectionCount;
...
router.get('/searchrecordsdefault/', function(req, res) {
    ...
    var sortOptions = { sort : { Artist : 1 } };

    collection.find(searchRequest, sortOptions, function(e,docs){
        res.json(docs);
    });
});
